...
negative control, and one `-C llvm-args=-fp-contract=fast` variant) and
asserts instruction families with width-aware minimum counts. REQUIRED checks
...
- Architectures are data-driven in the script (`ARCHES` maps each arch to
  its rows and expectation table). x86_64 is fully populated; aarch64 is
  scaffolded — rows reserved (NEON is the aarch64 baseline, so there is no
  scalar-only negative-control row), expectations empty — and such arches
  run in informational mode: probes compile and report, nothing fails, until
  a table is authored on that arch (`--hist` prints per-probe instruction
  histograms, `--dump` shows single bodies). Cross-emission from another
  host is deliberately not wired up.
